...
// BEGIN EXTRA CODE
function clickButton(buttonName, wrapperClassName) {
	//Get buttonElement. The mx-name is part of the class name

	const buttonElement = document.querySelector('.'+ wrapperClassName.trim() + ' button[class*="' + buttonName + '"]');
 ...
 * @param {string} wrapperClassName - Name of the unique class put on the container around the text area and the button
 ...
 */
export async function JS_Question_Textarea_ExecuteButtonAction(wrapperClassName, textAreaName, buttonName, submitOnEnter, submitOnShiftEnter) {
	// BEGIN USER CODE
	try {
		//Validations
		...
		if (!buttonName || wrapperClassName.trim().length === 0) {
			throw new Error("WrapperClassName is required.")
		}

		//Get textAreaElement. The mx-name is part of the ID.
		const textAreaElement = document.querySelector('.'+ wrapperClassName.trim() +' textarea[id*="' + textAreaName + '"]');
			...
			if (event.key === 'Enter') {
				if (event.shiftKey) {
					if (submitOnShiftEnter) {
						clickButton(buttonName, wrapperClassName);			
					}
