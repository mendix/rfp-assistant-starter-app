...
export async function TextBox_ExecuteButtonAction(textAreaName, buttonName, submitOnEnter, submitOnShiftEnter) {
	...
	try {
		//Validations
		if (!textBoxName || textBoxName.trim().length === 0) {
			throw new Error("TextBoxName is required.")
		}
		if (!buttonName || buttonName.trim().length === 0) {
			throw new Error("ButtonName is required.")
		}

		//Get textBoxElement. The mx-name is part of the ID.
		const textBoxElement = document.querySelector('input[id*="' + textBoxName + '"]');
		if (textBoxElement === null) {
			throw new Error("Textbox with that name could not be found.")
		}
